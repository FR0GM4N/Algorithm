def divide(n, s_x, s_y):  # 한변크기, 시작좌표
    global res
    if n == 0:
        return
    flag_0, flag_1 = 0, 0
    ...
    if not flag_0 and flag_1:  # 전부 1이면 True -> 1 반환
        res += '1'
        return
    elif flag_0 and not flag_1:  # 전부 0이면 False -> 0 반환
        res += '0'
        return
    elif flag_0 and flag_1:  # 0,1 섞인 경우 -> 쪼개기
        res += '('
        divide(n // 2, s_x, s_y)
